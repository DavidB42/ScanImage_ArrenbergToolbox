function ArrenbergSendTcpFrame(source,event,arguments)
   ...
   %{
    %convert all pixels to positive numbers
    minVal = min(imageXY(:));
    imgdataPositive = imageXY;
    if (minVal < 0)
        imgdataPositive = imageXY - minVal;
    end

    %convert to 16bit
    imgdataPositive = uint16(imgdataPositive);
    imgSendData = imgdataPositive(:);
    imgSendData = reshape(imgSendData, 1, width*height);
